...
public class FileTransferStatusGateway extends TableGateway {
	...
	public FileTransferStatusGateway(Context context) {
		super(context, FileTransferStatus.CONTENT_URI,
				FileTransferStatusColumns.getInstance()
	...
	public DetailedState getState(Integer id) {
		...
		Cursor cursor = queryUri(SELECTION_BY_ID,
				new String[] { id.toString() });
		if (cursor.getCount() > 0) {
			cursor.moveToFirst();
			int colIndex = cursor
					.getColumnIndex(FileTransferStatusColumns.STATUS);
